...
in {
  ...
  abstract = ''
    I have been cursing at Java a lot lately. Over this past week, it’s
  ...
  body = [
    ...
    (p ''
      I have been cursing at Java a lot lately. Over this past week, it’s
      because I’ve been experiencing inexplicable compilation failures.
      I finally tracked down the cause and submitted this bug report.
    '')
    ...
    (p "Create files One.java and Two.java, and run \`javac *.java\`.")
    ...
    (p ''
      ...
      imports, and when a class has the same (case-insensitive) name as a
      package in which it resides. Avoiding either of these situations is
      sufficient to work around this bug.
    '')
    ...
    (p ''
      I don’t understand why no one else seems to be complaining about this
      bug, because lately I can’t seem to avoid it.
    '')

    (p ''
      Perhaps I was too hasty in posting the bug report a few days ago,
      because I’ve discovered now that it happens in more cases than I thought.
      A namespace clash is not even necessary. You can change the package name
